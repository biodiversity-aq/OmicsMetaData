...
##Standardizing data
Another important objective that is embedded in the FAIR principles is data interoperability, which can only be achieved by standardizing datasets over many users. Therfore, this package offers a suite of tools to standardize datasets using the MIxS and DarwinCore data standards. For details on these standards and their implementation, see the [Background vignette](Background.Rmd). More information on MIxS, see https://gensc.org/mixs/ and https://github.com/GenomicsStandardsConsortium/mixs. For more information on DarwinCore, see https://dwc.tdwg.org and https://github.com/tdwg/dwc. For detailed examples on standardizing metadata, see the [Metadata_standardization vignette](Metadata_standardization.Rmd).
...
... complete a list of taxonomic names by looking-up missing information on the World Registry of Marine Species (WoRMS), using the WoRMS API and the worrms R-package. More information here: https://www.marinespecies.org/aphia.php?p=webservice&type=r
```{r worms, eval = FALSE}
...
##Archiving data
Archiving data, and getting in the right format and standard can be very challenging to say the least. Depending on the platform or database where the sequence data and metadata is archived, different requirements can be set and subtle variations to datastandards implemented.
